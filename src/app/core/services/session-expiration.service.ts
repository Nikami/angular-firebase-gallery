...
export class SessionExpirationService {
  ...
  private subscribeToAuthSession(): void {
    ...
    this.auth
      .get(AUTH_SUBJECT.SESSION)
      .subscribe((session: string) => this.setSessionExpiration(session));
  }

  private subscribeToSessionExpiration(): void {
    this.sessionExpirationSubscription = this.sessionExpiration$
      .pipe(
        tap((expMs: number) => {
          if (expMs !== null) {
            timer(expMs - ONE_MINUTE_MS).subscribe(() =>
  ...
  private openSessionExpirationModal(): void {
    ...
    const dialogRef: MatDialogRef<SessionDialogComponent> = this.dialog.open(
